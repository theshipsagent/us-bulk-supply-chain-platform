...
CREATE TABLE operators (
    ...
    -- Coded fields (foreign keys to lookup tables)
    dist_code INTEGER REFERENCES lookup_district(dist_code),
    series_code INTEGER REFERENCES lookup_series(series_code),
    service_code INTEGER REFERENCES lookup_service(service_code),
...
CREATE TABLE vessels (
    ...
    vtcc_code VARCHAR(4) REFERENCES lookup_vtcc(vtcc_code),
    icst_code VARCHAR(3) REFERENCES lookup_icst(icst_code),
    ...
    -- Capacity
    cap_ref_code VARCHAR(1) REFERENCES lookup_capacity_ref(cap_ref_code),
    cap_pass INTEGER,
    ...
    -- Equipment
    equip1 VARCHAR(20) REFERENCES lookup_equipment(equipment_code),
    equip2 VARCHAR(20) REFERENCES lookup_equipment(equipment_code),
    ...
    ts_oper INTEGER REFERENCES operators(ts_oper),
...
CREATE INDEX idx_analytics_operator ON vessel_analytics_view(ts_oper);
CREATE INDEX idx_analytics_operator_name ON vessel_analytics_view(operator_name);
CREATE INDEX idx_analytics_vtcc ON vessel_analytics_view(vtcc_code);
CREATE INDEX idx_analytics_icst ON vessel_analytics_view(icst_code);
CREATE INDEX idx_analytics_age_category ON vessel_analytics_view(age_category);
CREATE INDEX idx_analytics_district ON vessel_analytics_view(dist_code);
CREATE INDEX idx_analytics_series ON vessel_analytics_view(series_code);
CREATE INDEX idx_analytics_vessel_type ON vessel_analytics_view(vtcc_vessel_type);
CREATE INDEX idx_analytics_is_barge ON vessel_analytics_view(is_barge);
CREATE INDEX idx_analytics_is_self_propelled ON vessel_analytics_view(is_self_propelled);
CREATE INDEX idx_analytics_operator_state ON vessel_analytics_view(operator_state);

-- Function to refresh the materialized view
CREATE OR REPLACE FUNCTION refresh_vessel_analytics()
RETURNS void AS $$
BEGIN
    REFRESH VIEW CONCURRENTLY vessel_analytics_view;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION refresh_vessel_analytics IS 'Refresh the vessel analytics materialized view after data updates';

-- ============================================================
-- HELPER VIEWS FOR COMMON QUERIES
-- ============================================================

-- View: Top Operators by Fleet Size
CREATE OR REPLACE VIEW v_top_operators AS
SELECT
    op.ts_oper,
    op.operator_name,
    op.city || ', ' || op.state AS location,
    op.phone,
    op.principal_commodity,
    COUNT(v.vessel_id) AS vessel_count,
    AVG(v.nrt) AS avg_nrt,
    SUM(CASE WHEN vtcc.is_self_propelled THEN 1 ELSE 0 END) AS self_propelled_count,
    SUM(CASE WHEN vtcc.is_barge THEN 1 ELSE 0 END) AS barge_count,
    d.district_name,
    s.series_name
FROM operators op
LEFT JOIN vessels v ON op.ts_oper = v.ts_oper
LEFT JOIN lookup_vtcc vtcc ON v.vtcc_code = vtcc.vtcc_code
LEFT JOIN lookup_district d ON op.dist_code = d.dist_code
LEFT JOIN lookup_series s ON op.series_code = s.series_code
GROUP BY op.ts_oper, op.operator_name, op.city, op.state, op.phone,
         op.principal_commodity, d.district_name, s.series_name
HAVING COUNT(v.vessel_id) > 0
ORDER BY vessel_count DESC;

COMMENT ON VIEW v_top_operators IS 'Operators ranked by fleet size with contact info';
...
SELECT
    ...
    SUM(CASE WHEN vtcc.is_self_propelled THEN 1 ELSE 0 END) AS self_propelled,
    SUM(CASE WHEN vtcc.is_barge THEN 1 ELSE 0 END) AS barges,
    AVG(nrt) AS avg_nrt,
...
COMMENT ON VIEW v_fleet_summary IS 'High-level fleet statistics';

-- ============================================================
-- GRANTS (adjust based on your PostgreSQL user setup)
-- ============================================================

-- Grant read access to analytics view for dashboard user
-- GRANT SELECT ON vessel_analytics_view TO dashboard_user;
-- GRANT SELECT ON v_top_operators TO dashboard_user;
-- GRANT SELECT ON v_fleet_summary TO dashboard_user;
