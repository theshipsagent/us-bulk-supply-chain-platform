#!/usr/bin/env python3
"""
Slag ATLAS CLI - Command Line Interface
=========================================
Click-based CLI for Slag/GGBFS ATLAS project operations.
...
Commands:
    ...
    mills    - Show steel mill data (BF-BOF vs EAF)
"""
...
@click.pass_context
def cli(ctx, config):
    """Slag ATLAS - Analytical Tool for Logistics, Assets, Trade & Supply (Slag/GGBFS)"""
...
def refresh(ctx, source):
    ...
    Sources:
        epa      - EPA FRS facilities (steel mills, cement plants, slag processors) [default]
    ...
    click.echo("=" * 70)
    click.echo("SLAG ATLAS DATA REFRESH")
    click.echo("=" * 70)
    ...
    try:
        if 'epa' in sources_to_load:
            ...
            naics_config_path = str(Path(__file__).parent / config['naics']['slag_config'])

            click.echo(f"[FRS] Loading EPA FRS data for slag/GGBFS...")
...
@click.option('--category', type=click.Choice([
    'Steel Production (BF-BOF)', 'Steel Production (EAF)',
    'Cement/Concrete End User', 'Slag Processing', 'Steel Downstream'
...
def query(ctx, state, naics, company, category, limit, output):
    ...
    Examples:
        slag-atlas query --state IN
        slag-atlas query --category "Steel Production (BF-BOF)"
        slag-atlas query --company "Cleveland-Cliffs"
        slag-atlas query --naics 331111
    """
    config = ctx.obj['config']
    atlas_db_path = str(Path(__file__).parent / config['data']['atlas']['path'])
...
def stats(ctx, by_state, by_naics, by_company, by_category):
    ...
    try:
        ...
        click.echo("=" * 70)
        click.echo("SLAG ATLAS DATABASE STATISTICS")
        click.echo("=" * 70)
...
@cli.command()
@click.option('--bf-bof', is_flag=True, help='Show integrated BF-BOF mills (GGBFS source)')
@click.option('--eaf', is_flag=True, help='Show EAF mills (no GGBFS)')
@click.option('--all-mills', 'show_all', is_flag=True, help='Show all steel mills')
@click.pass_context
def mills(ctx, bf_bof, eaf, show_all):
    """Show steel mill data — BF-BOF vs EAF.

    BF-BOF integrated mills produce GGBFS; EAF mills do NOT.

    Examples:
        slag-atlas mills --bf-bof    # GGBFS source mills
        slag-atlas mills --eaf       # Non-GGBFS mills
        slag-atlas mills --all-mills # All steel mills
    """
    config = ctx.obj['config']
    atlas_db_path = str(Path(__file__).parent / config['data']['atlas']['path'])
    ...
    try:
        ...
        if bf_bof:
            click.echo("=" * 70)
            click.echo("INTEGRATED BF-BOF STEEL MILLS (GGBFS SOURCE)")
            click.echo("=" * 70)
            df = analytics.get_steel_mills(mill_type='bf-bof')
        elif eaf:
            click.echo("=" * 70)
            click.echo("EAF STEEL MILLS (NO GGBFS)")
            click.echo("=" * 70)
            df = analytics.get_steel_mills(mill_type='eaf')
        elif show_all:
            click.echo("=" * 70)
            click.echo("ALL STEEL MILLS")
            click.echo("=" * 70)
            df = analytics.get_steel_mills()
        else:
            # Default: summary
            click.echo("=" * 70)
            click.echo("STEEL MILL SUMMARY")
            click.echo("=" * 70)

            bf = analytics.get_steel_mills(mill_type='bf-bof')
            eaf_df = analytics.get_steel_mills(mill_type='eaf')

            click.echo(f"\nBF-BOF (integrated, GGBFS source):  {len(bf):,} facilities")
            click.echo(f"EAF (electric arc, NO GGBFS):       {len(eaf_df):,} facilities")
            click.echo(f"Total Steel:                        {len(bf) + len(eaf_df):,} facilities")
            click.echo("\nUse --bf-bof, --eaf, or --all-mills for details")
            return
        ...
        click.secho(f"[ERROR] Error: {e}", fg='red', err=True)
        logger.exception("Mills command failed")
        sys.exit(1)
...
def info(ctx, registry_id, state):
    ...
    try:
        if state:
            ...
            click.echo("Examples:")
            click.echo("  slag-atlas info 110000123456")
            click.echo("  slag-atlas info --state IN")
