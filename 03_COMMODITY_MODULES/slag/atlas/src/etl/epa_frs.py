#!/usr/bin/env python3
"""
EPA FRS Data Loader Module — Slag / GGBFS
============================================
Extracts slag-relevant facilities from EPA FRS database and loads to ATLAS.
Adapted from aggregates/atlas/src/etl/epa_frs.py for slag NAICS codes.

Key distinction: categorizes steel mills as BF-BOF (GGBFS source) vs EAF (no GGBFS).
Also captures cement/concrete end users and slag processing operations.
"""
...
# NAICS category mapping for slag supply chain
NAICS_CATEGORIES = {
    "331110": "Steel Production (BF-BOF)",
    "331111": "Steel Production (BF-BOF)",
    "331112": "Steel Production (EAF)",
    "327310": "Cement/Concrete End User",
    "327320": "Cement/Concrete End User",
    "327390": "Cement/Concrete End User",
    "212399": "Slag Processing",
    "562998": "Slag Processing",
    "331210": "Steel Downstream",
}


class EPAFRSLoader:
    """Handles extraction and loading of EPA FRS data for slag/GGBFS facilities."""
    ...
    def extract_facilities(
        ...
        """Extract facilities with slag-relevant NAICS codes."""
        logger.info("Extracting slag-relevant facilities by NAICS codes...")
    ...
    def _categorize_naics(self, naics_codes_str: str) -> str:
        ...
        Priority: Steel BF-BOF > Steel EAF > Slag Processing > Cement/Concrete > Other
        """
    ...
    def load_to_atlas(self, df: pd.DataFrame) -> None:
        """Load extracted facilities to ATLAS Slag database."""
        logger.info(f"Loading {len(df):,} records to ATLAS Slag database...")
        ...
        try:
            ...
            logger.info(f"Successfully loaded {row_count:,} facilities to ATLAS Slag")
    ...
    def refresh(self) -> None:
        ...
        logger.info("Starting EPA FRS data refresh for slag/GGBFS...")
        ...
        try:
            ...
            if df.empty:
                logger.warning("No facilities found with slag-relevant NAICS codes")
                return

            self.load_to_atlas(df)
            logger.info("EPA FRS data refresh complete for slag")
        finally:
            con.close()


def refresh_frs_data(frs_db_path: str, atlas_db_path: str, config_path: str = None) -> None:
    """Refresh FRS data in ATLAS Slag database."""
