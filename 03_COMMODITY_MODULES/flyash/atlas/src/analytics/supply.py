#!/usr/bin/env python3
"""
Supply Analytics Module — Fly Ash / CCP
==========================================
Query and analytics for fly ash facilities (coal power plants, ash processors,
cement/concrete end users, distributors).

Key feature: distinguishes coal power plants (fly ash source) from ash
processors/harvesters and cement/concrete end users.
"""
...
class SupplyAnalytics:
    """Handles analytics and queries for fly ash/CCP facility supply data."""
    ...
    def count_by_category(self) -> pd.DataFrame:
        """Count facilities by NAICS category (Coal Power, Ash Processing, etc.)."""
        con = self._connect()
        try:
            return con.execute("""
    ...
    def get_coal_plants(self, plant_type: Optional[str] = None) -> pd.DataFrame:
        """
        Get coal power generation facilities (fly ash source).

        Args:
            plant_type: 'coal' for coal power plants only,
                       'ash-processors' for ash processing/harvesting facilities,
                       None for all coal power + ash processing facilities
        """
        con = self._connect()
        try:
            if plant_type == 'coal':
                where = "WHERE naics_category = 'Coal Power (Fly Ash Source)'"
            elif plant_type == 'ash-processors':
                where = "WHERE naics_category = 'Ash Processing/Harvesting'"
            else:
                where = "WHERE naics_category IN ('Coal Power (Fly Ash Source)', 'Ash Processing/Harvesting')"
    ...
    def production_summary(self) -> Dict:
        ...
        stats['reference'] = {
            'total_ccp_mt': 63_600_000,
            'fly_ash_to_concrete_mt': 14_700_000,
            'beneficial_use_rate_pct': 72,
            'us_coal_capacity_gw': 165,
            'us_price_per_mt': 123,
            'source': 'ACAA 2024 Survey, EIA, industry estimates',
            'year': 2024,
            'unit': 'short_tons',
        }
