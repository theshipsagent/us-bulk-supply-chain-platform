#!/usr/bin/env python3
"""
Supply Analytics Module — Natural Pozzolan
=============================================
Query and analytics for pozzolan facilities (mines, processing plants,
cement/concrete end users).

Key feature: distinguishes Pozzolan Mining from Limestone Mining (LC3)
and Pozzolan Processing operations.
"""
...
class SupplyAnalytics:
    """Handles analytics and queries for natural pozzolan facility supply data."""
    ...
    def count_by_category(self) -> pd.DataFrame:
        """Count facilities by NAICS category (Mining, Processing, etc.)."""
        con = self._connect()
        try:
            return con.execute("""
    ...
    def get_mining_operations(self, mine_type: Optional[str] = None) -> pd.DataFrame:
        """
        Get pozzolan mining/processing operations.

        Args:
            mine_type: 'pozzolan' for pumice/volcanic ash/clay mines,
                      'limestone' for limestone quarries (LC3 feedstock),
                      'processing' for pozzolan processing facilities,
                      None for all mining/processing operations
        """
        con = self._connect()
        try:
            if mine_type == 'pozzolan':
                where = "WHERE naics_category = 'Pozzolan Mining'"
            elif mine_type == 'limestone':
                where = "WHERE naics_category = 'Limestone Mining (LC3)'"
            elif mine_type == 'processing':
                where = "WHERE naics_category = 'Pozzolan Processing'"
            else:
                where = "WHERE naics_category IN ('Pozzolan Mining', 'Limestone Mining (LC3)', 'Pozzolan Processing')"
    ...
    def production_summary(self) -> Dict:
        ...
        stats['reference'] = {
            'npa_shipments_2024_mt': 2_500_000,
            'projected_capacity_2028_mt': 3_500_000,
            'price_per_mt': 115,
            'fly_ash_price_per_mt': 118,
            'lc3_doe_investment_usd': 1_600_000_000,
            'source': 'NPA, USGS MCS 2025, DOE OCED',
            'year': 2024,
            'unit': 'short_tons',
        }
