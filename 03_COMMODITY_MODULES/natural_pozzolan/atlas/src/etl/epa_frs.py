#!/usr/bin/env python3
"""
EPA FRS Data Loader Module — Natural Pozzolan
================================================
Extracts pozzolan-relevant facilities from EPA FRS database and loads to ATLAS.
Adapted from slag/atlas/src/etl/epa_frs.py for pozzolan NAICS codes.

Key distinction: categorizes facilities as Pozzolan Mining, Limestone Mining (LC3),
Pozzolan Processing, or Cement/Concrete End User.
"""
...
# NAICS category mapping for pozzolan supply chain
NAICS_CATEGORIES = {
    "212399": "Pozzolan Mining",
    "212325": "Pozzolan Mining",
    "212312": "Limestone Mining (LC3)",
    "327999": "Pozzolan Processing",
    "325199": "Pozzolan Processing",
...
class EPAFRSLoader:
    """Handles extraction and loading of EPA FRS data for natural pozzolan facilities."""
    ...
    def _load_naics_codes(self) -> List[str]:
        ...
        if self.config_path and Path(self.config_path).exists():
            ...
            return [
                "2123",    # Broad: Nonmetallic Mineral Mining
                "3273",    # Broad: Cement and Concrete
                "212399", "212325", "212312",
                "327999", "325199",
    ...
    def extract_facilities(
        ...
        """Extract facilities with pozzolan-relevant NAICS codes."""
        logger.info("Extracting pozzolan-relevant facilities by NAICS codes...")
    ...
    def _categorize_naics(self, naics_codes_str: str) -> str:
        ...
        Priority: Pozzolan Mining > Limestone Mining > Pozzolan Processing > Cement/Concrete > Other
        """
    ...
    def load_to_atlas(self, df: pd.DataFrame) -> None:
        """Load extracted facilities to ATLAS Pozzolan database."""
        logger.info(f"Loading {len(df):,} records to ATLAS Pozzolan database...")
        ...
        try:
            ...
            logger.info(f"Successfully loaded {row_count:,} facilities to ATLAS Pozzolan")
    ...
    def refresh(self) -> None:
        ...
        logger.info("Starting EPA FRS data refresh for natural pozzolans...")
        ...
        try:
            ...
            if df.empty:
                logger.warning("No facilities found with pozzolan-relevant NAICS codes")
                return

            self.load_to_atlas(df)
            logger.info("EPA FRS data refresh complete for natural pozzolans")
        finally:
            con.close()


def refresh_frs_data(frs_db_path: str, atlas_db_path: str, config_path: str = None) -> None:
    """Refresh FRS data in ATLAS Pozzolan database."""
