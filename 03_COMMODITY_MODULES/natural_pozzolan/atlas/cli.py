#!/usr/bin/env python3
"""
Natural Pozzolan ATLAS CLI - Command Line Interface
=====================================================
Click-based CLI for Natural Pozzolan ATLAS project operations.
...
Commands:
    ...
    deposits - Show pozzolan mining/processing operations
"""
...
@click.pass_context
def cli(ctx, config):
    """Natural Pozzolan ATLAS - Analytical Tool for Logistics, Assets, Trade & Supply (Pozzolans/LC3)"""
...
def refresh(ctx, source):
    ...
    Sources:
        epa      - EPA FRS facilities (pozzolan mines, processing, cement plants) [default]
    ...
    click.echo("=" * 70)
    click.echo("NATURAL POZZOLAN ATLAS DATA REFRESH")
    click.echo("=" * 70)
    ...
    try:
        if 'epa' in sources_to_load:
            ...
            naics_config_path = str(Path(__file__).parent / config['naics']['pozzolan_config'])

            click.echo(f"[FRS] Loading EPA FRS data for natural pozzolans...")
...
@click.option('--category', type=click.Choice([
    'Pozzolan Mining', 'Limestone Mining (LC3)',
    'Pozzolan Processing', 'Cement/Concrete End User'
...
def query(ctx, state, naics, company, category, limit, output):
    ...
    Examples:
        pozzolan-atlas query --state ID
        pozzolan-atlas query --category "Pozzolan Mining"
        pozzolan-atlas query --company "Hess Pumice"
        pozzolan-atlas query --naics 212399
    """
    config = ctx.obj['config']
    atlas_db_path = str(Path(__file__).parent / config['data']['atlas']['path'])
...
def stats(ctx, by_state, by_naics, by_company, by_category):
    ...
    try:
        ...
        click.echo("=" * 70)
        click.echo("NATURAL POZZOLAN ATLAS DATABASE STATISTICS")
        click.echo("=" * 70)
...
@cli.command()
@click.option('--pozzolan', is_flag=True, help='Show pozzolan mining operations (pumice, volcanic ash, clay)')
@click.option('--limestone', is_flag=True, help='Show limestone mining for LC3')
@click.option('--processing', is_flag=True, help='Show pozzolan processing facilities')
@click.pass_context
def deposits(ctx, pozzolan, limestone, processing):
    """Show pozzolan mining and processing operations.

    Pozzolan mines produce pumice, volcanic ash, and clay; limestone quarries
    feed LC3 cement; processing facilities prepare pozzolanic materials.

    Examples:
        pozzolan-atlas deposits --pozzolan     # Pumice/volcanic ash/clay mines
        pozzolan-atlas deposits --limestone    # Limestone quarries for LC3
        pozzolan-atlas deposits --processing   # Processing facilities
    """
    config = ctx.obj['config']
    atlas_db_path = str(Path(__file__).parent / config['data']['atlas']['path'])
    ...
    try:
        ...
        if pozzolan:
            click.echo("=" * 70)
            click.echo("POZZOLAN MINING OPERATIONS (PUMICE, VOLCANIC ASH, CLAY)")
            click.echo("=" * 70)
            df = analytics.get_mining_operations(mine_type='pozzolan')
        elif limestone:
            click.echo("=" * 70)
            click.echo("LIMESTONE MINING FOR LC3 CEMENT")
            click.echo("=" * 70)
            df = analytics.get_mining_operations(mine_type='limestone')
        elif processing:
            click.echo("=" * 70)
            click.echo("POZZOLAN PROCESSING FACILITIES")
            click.echo("=" * 70)
            df = analytics.get_mining_operations(mine_type='processing')
        else:
            # Default: summary
            click.echo("=" * 70)
            click.echo("POZZOLAN DEPOSIT & PROCESSING SUMMARY")
            click.echo("=" * 70)

            poz = analytics.get_mining_operations(mine_type='pozzolan')
            lime = analytics.get_mining_operations(mine_type='limestone')
            proc = analytics.get_mining_operations(mine_type='processing')

            click.echo(f"\nPozzolan Mining (pumice/volcanic ash/clay):  {len(poz):,} facilities")
            click.echo(f"Limestone Mining (LC3 feedstock):             {len(lime):,} facilities")
            click.echo(f"Pozzolan Processing:                         {len(proc):,} facilities")
            click.echo(f"Total Mining/Processing:                     {len(poz) + len(lime) + len(proc):,} facilities")
            click.echo("\nUse --pozzolan, --limestone, or --processing for details")
            return
        ...
        click.secho(f"[ERROR] Error: {e}", fg='red', err=True)
        logger.exception("Deposits command failed")
        sys.exit(1)
...
def info(ctx, registry_id, state):
    ...
    try:
        if state:
            ...
            click.echo("Examples:")
            click.echo("  pozzolan-atlas info 110000123456")
            click.echo("  pozzolan-atlas info --state ID")
